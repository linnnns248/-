...
const LOCAL_SPECIALTIES = [
  { name: '老妈蹄花', image: 'https://images.unsplash.com/photo-1541544741938-0af808871cc0?auto=format&fit=crop&q=80&w=200', score: '9.8', level: '名吃榜首' },
  { name: '冷锅串串', image: 'https://images.unsplash.com/photo-1582878826629-29b7ad1cdc43?auto=format&fit=crop&q=80&w=200', score: '9.6', level: '必尝特色' },
  { name: '蛋烘糕', image: 'https://images.unsplash.com/photo-1626074353765-517a681e40be?auto=format&fit=crop&q=80&w=200', score: '9.9', level: '非遗名吃' },
  { name: '跷脚牛肉', image: 'https://images.unsplash.com/photo-1563379926898-05f4575a45d8?auto=format&fit=crop&q=80&w=200', score: '9.4', level: '珍味首选' },
  { name: '甜水面', image: 'https://images.unsplash.com/photo-1599321955726-e0484294ca2e?auto=format&fit=crop&q=80&w=200', score: '9.5', level: '嚼劲担当' },
];
...
export default function App() {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-[#F0F2F5] p-4 lg:p-10">
      {/* Tooltip Overlay */}
      {showTooltip && (
        <div 
      ...
      <div className="relative w-full max-w-[375px] h-[812px] bg-white rounded-[48px] shadow-[0_40px_100px_rgba(0,0,0,0.2)] overflow-hidden border-[10px] border-gray-900 ring-4 ring-white/20">
        ...
        >
          {/* --- Atmospheric Top Header (Dynamic Scene) --- */}
          <div className="absolute top-0 left-0 right-0 h-[320px] pointer-events-none isolate">
            <img 
              src={isNight 
                ? "https://images.unsplash.com/photo-1546702482-1dd38f6d8995?auto=format&fit=crop&q=80&w=800" 
                : "https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&q=80&w=800"} 
              alt="Chengdu Cityscape" 
              className={cn(
                "w-full h-full object-cover mix-blend-multiply transition-all duration-1000",
                isNight ? "opacity-30 brightness-50" : "opacity-40 brightness-110"
              )}
            ...
            <div className={cn(
              ...
              isNight 
                ? "bg-gradient-to-b from-indigo-900/60 via-purple-900/20 to-transparent" 
                : "bg-gradient-to-b from-brand-primary/80 via-brand-primary/10 to-transparent"
            )} />
            
            {/* Local Element: Floating Map/Architecture Hint */}
            <div className="absolute top-16 right-[-20px] opacity-10 blur-[1px] pointer-events-none transform rotate-12 scale-110">
               <MapIcon className="w-48 h-48 text-gray-900" />
            </div>
            
            {/* Scene Toggle Switch - Hidden in production usually, but here for "Day/Night" requirement */}
            <div className="absolute top-14 right-4 pointer-events-auto z-[60]">
               <button 
                onClick={() => setIsNight(!isNight)}
                className="w-8 h-8 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center border border-white/30"
               >
                 {isNight ? <Moon className="w-3.5 h-3.5 text-yellow-200" /> : <Sun className="w-3.5 h-3.5 text-orange-400" />}
               </button>
            </div>
          </div>
              ...
              <div className="bg-white rounded-[24px] shadow-md border border-gray-50 overflow-hidden relative isolate">
                ...
                <div className="px-4 mb-2">
                   <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
                      {TRENDING_KEYWORDS.map((word, i) => (
                        ...
                        >
                          <span className={cn("text-[7px]", i === 0 ? "text-red-500" : "text-gray-400")}>{i + 1}</span>
                          {word.text}
                ...
                <div className="px-3 pb-3">
                   <div className="flex items-center justify-between mb-1.5 px-0.5">
                      <span className="text-[10px] font-extrabold text-gray-900 border-l-2 border-brand-primary pl-1.5 uppercase tracking-tighter">当地特色</span>
                      <span className="text-[7px] text-gray-300 font-bold flex items-center gap-0.5 tracking-tighter">更多 <ChevronRight className="w-2 h-2" /></span>
                   </div>
                   <div className="flex gap-2 overflow-x-auto no-scrollbar snap-x">
                      {LOCAL_SPECIALTIES.map((spec, i) => (
                        <div key={i} className="flex flex-col gap-1 min-w-[85px] snap-center">
                           <div className="h-10 rounded-lg overflow-hidden relative border border-gray-100 bg-gray-50 flex-shrink-0">
                              <img 
                                src={spec.image} 
                                alt={spec.name} 
                              ...
                              <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center p-1">
                                 <h4 className="text-[8px] font-black text-white text-center leading-none mb-0.5">{spec.name}</h4>
                                 <span className="text-[6px] font-bold text-brand-primary uppercase tracking-tighter px-1 rounded bg-black/40 border border-white/5">
                                   {spec.level}
                                 </span>
                              </div>
                           </div>
                      ...
                      <div className="flex items-center justify-center min-w-[32px] h-10 rounded-lg bg-gray-50 border border-dashed border-gray-200">
